...
insert into accounting.productCategories values (nextVal('accounting.categoryIDSeq'),'Boissons chaudes','SUC','cafe.png',1, 'y');
...
create or replace view accounting.fullYearDays as
    select generate_series( (date '2020-01-01')::timestamp,
                            (date '2020-12-31')::timestamp,
                            interval '1 day'
                            ) as date;

create or replace view accounting.fullMonthDays as
    select extract(day from date) as day from accounting.fullYearDays where extract(month from date) = 1;
